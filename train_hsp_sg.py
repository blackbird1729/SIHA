...
if __name__ == '__main__':
    ...
    for batch in train_loader:
        ...
        for i in range(data.shape[0]):
            cached_adj_list.append(adj[i].cpu())
    ...
    for batch in valid_loader:
        ...
        for i in range(data.shape[0]):
            cached_adj_list_val.append(adj[i].cpu())
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, (data, relations) in enumerate(train_loader):
            ...
            if update_structure:
                predictor.eval()
                with torch.no_grad():
                    update_pred_hidden = predictor(visible_data, cached_adj_batch)
        ...
        with torch.no_grad():
            for batch_idx, (data, relations) in enumerate(valid_loader):
                ...
                if update_structure:
                    with torch.no_grad():
                        update_pred_hidden = predictor(visible_data, cached_adj_batch_val)
                        update_pred_hidden_aligned, perm = hungarian_match(update_pred_hidden, true_hidden)
                        update_data_all = torch.cat([visible_data, update_pred_hidden_aligned], dim=1)
                        update_logits =ss* pre_nri_enc(update_data_all, rel_rec, rel_send)
                        update_pred_relations = torch.argmax(update_logits, dim=-1)
