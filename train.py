...
parser.add_argument('--num-atoms', type=int, default=9,
                    help='Number of atoms in simulation.')
parser.add_argument('--num-hidden', type=int, default=4,
                    help='Number of hidden atoms in simulation.')
...
parser.add_argument('--suffix', type=str, default='_charged9',
                    help='Suffix for training data (e.g. "_charged".')
parser.add_argument('--encoder-dropout', type=float, default=0.0,
                    help='Dropout rate (1 - keep probability).')
parser.add_argument('--decoder-dropout', type=float, default=0.0,
                    help='Dropout rate (1 - keep probability).')
parser.add_argument('--save-folder', type=str, default='C:\hidden_node\logs',
...
print(args)
num_visible=args.num_atoms-args.num_hidden
...
off_diag = np.ones([num_visible, num_visible]) - np.eye(num_visible)
...
def get_visible_off_diag_idx(num_atoms, num_visible):
    full_off_diag = np.ravel_multi_index(
        np.where(np.ones((num_atoms, num_atoms)) - np.eye(num_atoms)),
        [num_atoms, num_atoms]
    )
    # 提取可见节点之间的索引
    visible_off_diag = np.ravel_multi_index(
        np.where(np.ones((num_visible, num_visible)) - np.eye(num_visible)),
        [num_atoms, num_atoms]  # 注意仍需映射到完整 num_atoms 范围
    )
    for i in range(len(visible_off_diag)):
        visible_off_diag[i]-=i//(num_visible)+1
    return visible_off_diag

visible_off_diag = get_visible_off_diag_idx(args.num_atoms, num_visible)
print(visible_off_diag)
...
def train(epoch, best_val_loss):
    ...
    for batch_idx, (data, relations) in enumerate(train_loader):
        ...
        visible_data = data[:, :num_visible, :, :]

        visible_relations = relations[:, visible_off_diag]

        optimizer.zero_grad()


        logits = encoder(visible_data, rel_rec, rel_send)
        ...
        if args.decoder == 'rnn':
            output = decoder(visible_data, edges, rel_rec, rel_send, 100,
                             burn_in=True,
                             burn_in_steps=args.timesteps - args.prediction_steps)
        else:
            output = decoder(visible_data, edges, rel_rec, rel_send,
                             args.prediction_steps)

        target = visible_data[:, :, 1:, :]
        ...
        acc = edge_accuracy(logits, visible_relations)
    ...
    for batch_idx, (data, relations) in enumerate(valid_loader):
        ...
        output = decoder(visible_data, edges, rel_rec, rel_send, 1)
...
def test():
    ...
    for batch_idx, (data, relations) in enumerate(test_loader):
        ...
        data_encoder = data[:, :num_visible, :args.timesteps, :].contiguous()
        data_decoder = data[:, :num_visible, -args.timesteps:, :].contiguous()
        visible_relations = relations[:, visible_off_diag]
        ...
        target = data_decoder[:, :num_visible, 1:, :]
        loss_nll = nll_gaussian(output, target, args.var)
        loss_kl = kl_categorical_uniform(prob, args.num_atoms, args.edge_types)
        ...
        if args.decoder == 'rnn':
            ...
            data_plot = data[:, :num_visible, args.timesteps:args.timesteps + 21,
                        :].contiguous()
            output = decoder(data_plot, edges, rel_rec, rel_send, 20)
            target = data_plot[:, :num_visible, 1:, :]
