...
def load_kuramoto_data(batch_size=1, suffix=''):
    feat_train = np.load('kuramoto_data/feat_train' + suffix +'undir'+ '.npy')
    edges_train = np.load('kuramoto_data/edges_train' + suffix + 'undir'+'.npy')
    feat_valid = np.load('kuramoto_data/feat_valid' + suffix +'undir'+ '.npy')
    edges_valid = np.load('kuramoto_data/edges_valid' + suffix + 'undir'+'.npy')
    feat_test = np.load('kuramoto_data/feat_test' + suffix + 'undir'+'.npy')
    edges_test = np.load('kuramoto_data/edges_test' + suffix + 'undir'+'.npy')
...
def unflatten_adj_matrix(flat_adj, num_nodes):
    B = flat_adj.size(0)
    device = flat_adj.device
    dtype = flat_adj.dtype  # <- 自动匹配原始数据类型

    full_adj = torch.zeros(B, num_nodes, num_nodes, device=device, dtype=dtype)

    off_diag_mask = ~torch.eye(num_nodes, dtype=torch.bool, device=device)
    full_adj[:, off_diag_mask] = flat_adj

    return full_adj

def flatten_adj_matrix(adj):
    """
    去除邻接矩阵的对角线元素，并展平为 [B, N*(N-1)]。

    Args:
        adj: [B, N, N]，邻接矩阵（通常对称）

    Returns:
        flat_adj: [B, N*(N-1)]，去除对角后的展平矩阵
    """
    B, N, _ = adj.shape
    # 创建一个 [N, N] 的 mask，去除对角线
    mask = ~torch.eye(N, dtype=torch.bool, device=adj.device)  # [N, N]
    flat_adj = adj[:, mask]  # [B, N*(N-1)]
    return flat_adj

def hungarian_match(pred, target):
    from scipy.optimize import linear_sum_assignment
    B, H, T, D = pred.shape
    pred_matched = torch.zeros_like(pred)
    l_perm = torch.zeros(B, H, dtype=torch.long)

    for b in range(B):
        cost_matrix = torch.cdist(
            pred[b].reshape(H, -1),
            target[b].reshape(H, -1),
            p=2
        )
        row_ind, col_ind = linear_sum_assignment(cost_matrix.cpu().detach().numpy())

        perm = torch.argsort(torch.tensor(col_ind))
        pred_matched[b] = pred[b, perm]
        l_perm[b]=perm

    return pred_matched, l_perm.to(pred.device)

def align_structure(adj_pred, perm, V):
    """
    对预测的邻接矩阵中的隐藏节点部分进行对齐。

    Args:
        adj_pred: [B, N, N] 预测邻接矩阵
        perm: [B, H] 每个样本的隐藏节点排列索引（例如来自匈牙利匹配）
        V: 可见节点数

    Returns:
        aligned_adj_pred: [B, N, N] 对齐后的预测邻接矩阵
    """
    B, N, _ = adj_pred.shape
    H = N - V

    aligned = adj_pred.clone()

    for b in range(B):
        p = perm[b]  # [H]
        # 对隐藏部分的行重新排序
        aligned[b, V:, :] = aligned[b, V + p, :]         # 行：隐藏节点重排
        aligned[b, :, V:] = aligned[b, :, V + p]         # 列：隐藏节点重排
    return aligned

def create_masks(n, m):
    # 初始化全零mask
    mask_top_left = torch.zeros((n, n), dtype=torch.bool)
    mask_bottom_right = torch.zeros((n, n), dtype=torch.bool)

    # 左上角 m x m 区域设置为 True
    mask_top_left[:m, :m] = True

    # 右下角 (n-m) x (n-m) 区域设置为 True
    start = n - m
    mask_bottom_right[start:, start:] = True

    # 其余部分是不在以上两个区域的部分
    mask_rest = ~(mask_top_left | mask_bottom_right)

    return mask_top_left, mask_bottom_right, mask_rest

def block_fill(x: torch.Tensor, M: int) -> torch.Tensor:
    """就地修改 x（形状 [B,N,N]，元素 0/1）。"""
    B, N, _ = x.shape
    # 左下：若某列含 1，则整列置 1
    col_has_one = x[:, M:, :M].any(dim=1)                      # [B, M]
    col_mask    = col_has_one.unsqueeze(1).expand(-1, N-M, -1)
    x[:, M:, :M] = torch.where(col_mask, 1, x[:, M:, :M])

    # 右上：若某行含 1，则整行置 1
    row_has_one = x[:, :M, M:].any(dim=2)                      # [B, M]
    row_mask    = row_has_one.unsqueeze(2).expand(-1, -1, N-M)
    x[:, :M, M:] = torch.where(row_mask, 1, x[:, :M, M:])

    return x

def flatten_adj_tensor(adj):
    """
    去除邻接张量的对角线元素并展平为 [B, N*(N-1), D]。
    支持 [B, N, N, D] 输入。
    ...
    B, N, _, D = adj.shape
    mask = ~torch.eye(N, dtype=torch.bool, device=adj.device)  # [N, N]
    mask = mask.unsqueeze(-1).expand(N, N, D)  # [N, N, D]
    flat_adj = adj[:, mask].view(B, N * (N - 1), D)
    return flat_adj

def unflatten_adj_tensor(flat_adj, num_nodes):
    """
    将展平的邻接张量恢复为 [B, N, N, D]，自动补回对角线为 0。
    ...
    Returns:
        full_adj: [B, N, N, D]
    """
    B, L, D = flat_adj.shape
    assert L == num_nodes * (num_nodes - 1)

    full_adj = torch.zeros(B, num_nodes, num_nodes, D, device=flat_adj.device, dtype=flat_adj.dtype)

    # 创建掩码：保留非对角线元素位置
    mask = ~torch.eye(num_nodes, dtype=torch.bool, device=flat_adj.device)  # [N, N]
    mask = mask.unsqueeze(-1).expand(num_nodes, num_nodes, D)  # [N, N, D]

    for b in range(B):
        full_adj[b][mask] = flat_adj[b].reshape(-1)

    return full_adj


def align_structure_tensor(adj, perm, num_visible):
    """
    输入:
        adj: [B, N, N, D] 的邻接张量
        perm: [B, H] 匹配的排列索引
        num_visible: V，可见节点数
    返回:
        aligned_adj: 与 perm 对齐后的邻接张量，仍为 [B, N, N, D]
    """
    B, N, _, D = adj.shape
    H = N - num_visible
    V = num_visible
    ...
    for b in range(B):
        ...
        # 行重排：隐藏节点部分对隐藏节点的出边重新排列
        aligned[b, V:V+H, :, :] = aligned[b, V + p, :, :]

        # 列重排：隐藏节点部分对隐藏节点的入边重新排列
        aligned[b, :, V:V+H, :] = aligned[b, :, V + p, :]

    return aligned

def compute_true_nums(adj, max_num_hidden):
    if not isinstance(adj, torch.Tensor):
        adj = torch.tensor(adj)  # 确保是 tensor
    ...
    k_tensor = torch.tensor(k_values, dtype=torch.int32, device=device)
    true_nums = max_num_hidden - torch.clamp(k_tensor, max=max_num_hidden)
    return true_nums

def get_off_diag_idx(num_atoms, num_visible):
    v2v=[]
    v2h=[]
    h2h=[]
    dig=0
    num_hidden=num_atoms-num_visible
    for i in range(num_atoms**2):
        row_index, column_index = i//num_atoms, i % num_atoms
        # print(row_index, column_index,i)
        if row_index == column_index:
            dig+=1
            continue
        if row_index < num_visible and column_index < num_visible:
            v2v.append(i-dig)
        elif row_index >= num_visible and column_index >= num_visible:
            h2h.append(i-dig)
        else:
            v2h.append(i-dig)
    return v2v, v2h, h2h